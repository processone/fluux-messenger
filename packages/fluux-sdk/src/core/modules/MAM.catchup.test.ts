...
describe('MAM Background Catch-Up', () => {
  ...
  describe('catchUpAllConversations', () => {
    it('should do nothing when there are no conversations', async () => {
      ...
      await getInternalSurfaceForTesting(xmppClient).mam.catchUpAllConversations()
    ...
    it('should query with forward start filter for conversations with cached messages', async () => {
      ...
      const catchUpPromise = getInternalSurfaceForTesting(xmppClient).mam.catchUpAllConversations()
    ...
    it('should respect concurrency limit', async () => {
      ...
      const catchUpPromise = getInternalSurfaceForTesting(xmppClient).mam.catchUpAllConversations({ concurrency: 2 })
    ...
    it('should skip excluded conversation', async () => {
      ...
      const catchUpPromise = getInternalSurfaceForTesting(xmppClient).mam.catchUpAllConversations({ exclude: 'bob@example.com' })
      await waitForAsyncOps(30, 100)
      await catchUpPromise
    ...
    it('should handle null exclude gracefully', async () => {
      ...
      const catchUpPromise = getInternalSurfaceForTesting(xmppClient).mam.catchUpAllConversations({ exclude: null })
      await waitForAsyncOps(20, 100)
      await catchUpPromise
  ...
  describe('catchUpConversationHistory (latest-first orchestrator)', () => {
    ...
    it('stops the Phase B walk immediately when the backward cursor stops advancing (stuck pointer)', async () => {
      ...
      const calls: any[] = []
      const querySpy = vi.spyOn(getInternalSurfaceForTesting(xmppClient).mam, 'queryArchive').mockImplementation(async (opts: any) => {
        calls.push(opts)
    ...
    it('forward-complete Phase A + still-pending pointer: seeds the backward walk from the TRUE cache bottom', async () => {
      ...
      await getInternalSurfaceForTesting(xmppClient).mam.catchUpConversationHistory('alice@example.com', cached, {
    ...
    it('does NOT descend Phase B from the cache bottom when coverage is flagged unproven and no gap edge exists (finding 10)', async () => {
      ...
      // No backward query issued — the cache bottom probe is not trusted.
      const backward = calls.filter((c) => c.before !== undefined && c.before !== '')
      expect(backward).toHaveLength(0)
      expect(mockStores.chat.loadMessagesFromCache).not.toHaveBeenCalledWith(
        'alice@example.com', expect.objectContaining({ oldest: true }),
      )
    })
    ...
    it('degrades gracefully through the orchestrator when the Phase A anchor is purged (item-not-found)', async () => {
      ...
      await expect(
        getInternalSurfaceForTesting(xmppClient).mam.catchUpConversationHistory('alice@example.com', cached, {
          sessionStartTime: new Date('2026-06-14T12:00:00Z').getTime(),
        })
      ).resolves.not.toThrow()
  ...
  describe('catchUpRoomHistory (latest-first orchestrator, room twin)', () => {
    ...
    it('forward-complete Phase A + still-pending pointer: seeds the backward walk from the TRUE cache bottom', async () => {
      ...
      await getInternalSurfaceForTesting(xmppClient).mam.catchUpRoomHistory(roomJid, cached, {
        sessionStartTime: new Date('2026-06-14T12:00:00Z').getTime(),
        stitchReadPointer: true,
      })
    ...
    it('does NOT descend Phase B from the cache bottom when coverage is flagged unproven and no gap edge exists (finding 10)', async () => {
      ...
      const backward = calls.filter((c) => c.before !== undefined && c.before !== '')
      expect(backward).toHaveLength(0)
      expect(mockStores.room.loadMessagesFromCache).not.toHaveBeenCalledWith(
        roomJid, expect.objectContaining({ oldest: true }),
      )
    })
  ...
  describe('catchUpRoom (per-room forward cursor policy)', () => {
    ...
    it('does nothing while the connection is not online', async () => {
      ...
      await getInternalSurfaceForTesting(xmppClient).mam.catchUpRoom('room1@conference.example.com')
  ...
  describe('forceCatchUpAllRooms', () => {
    it('should do nothing when there are no joined rooms', async () => {
      ...
      await getInternalSurfaceForTesting(xmppClient).mam.forceCatchUpAllRooms()
    ...
    it('should emit console event with room count and days', async () => {
      ...
      const catchUpPromise = getInternalSurfaceForTesting(xmppClient).mam.forceCatchUpAllRooms({ days: 3 })
      await waitForAsyncOps(20, 100)
      await catchUpPromise
  ...
  describe('discoverNewConversationsFromRoster', () => {
    ...
    it('should query MAM for contacts without existing conversations', async () => {
      ...
      const discoverPromise = getInternalSurfaceForTesting(xmppClient).mam.discoverNewConversationsFromRoster()
      await waitForAsyncOps(20, 100)
