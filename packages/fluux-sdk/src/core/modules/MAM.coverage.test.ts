...
function harness(kind: Kind) {
  ...
  const row = (entry: Entry): Message | RoomMessage => {
    ...
    if (kind !== 'room') return { originId: undefined, ...common, type: 'chat', conversationId: PEER, from: PEER }
    ...
    const message: RoomMessage = { originId: undefined, occupantId: undefined, ...common, type: 'groupchat', roomJid: ROOM, from: `${ROOM}/peer`, nick: 'peer' }
    return { ...message }
  }
  if (kind === 'room') {
