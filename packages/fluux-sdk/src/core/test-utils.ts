...
/**
 * One `vi.fn()` per binding key. Derived from the same key lists as the
 * StoreBindings interface and createDefaultStoreBindings, so a newly listed
 * store method automatically gets a mock — no hand-maintained third copy.
 */
...
export const createMockStores = (): MockStoreBindings => ({
  ...
  console: mockMethods(consoleBindingMethodKeys),
  events: mockMethods(eventsBindingMethodKeys),
  room: {
    ...mockMethods(roomBindingMethodKeys),
  ...
  blocking: {
    ...mockMethods(blockingBindingMethodKeys),
...
export const createMockXMPPClientForHooks = () => ({
  ...
  profile: {
    ...
    fetchOwnProfileDetails: vi.fn(),
    publishOwnProfileDetails: vi.fn(),
