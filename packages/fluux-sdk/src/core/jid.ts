...
/**
 * Result of a local JID shape validation.
 *
 * `reason: 'empty'` lets a UI gate a submit button without nagging the user
...
/**
 * Local, network-free validation of a user-entered JID (e.g. a login field).
 *
...
export function validateBareJid(input: string): JidValidation {
  ...
  const domain = getBareJid(domainAndResource)
