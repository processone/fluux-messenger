...
- A message containing emoji no longer shifts what follows: reply quotes are no longer cut one character short and mention highlights land on the mention, in both directions
- A reaction to a poll or to a message that only carries a file now names what was reacted to, instead of quoting nothing
- Accepting a contact request no longer re-announces the messages they sent you before you accepted
- The contact list in the "New message" dialog is no longer cut off at the bottom of the dialog, and no longer puts a second scrollbar inside it
- A deleted message no longer turns up in search, and it no longer shows its old text anywhere Fluux summarises a conversation — the sidebar, notifications, the command palette and the clipboard all read "Message deleted"
- Removing your profile picture now clears it for your contacts too, instead of leaving your old picture showing in their client
...
- Copying several messages at once now includes polls and messages that only carry a file, instead of dropping them — and no longer falls back to a plain copy when the selection was mostly such messages
