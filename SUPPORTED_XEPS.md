...
| [XEP-0425](https://xmpp.org/extensions/xep-0425.html) | Message Moderation           | ✅ Implemented | Room moderation with v0/v1 broadcasts and archive tombstones; see [Messaging](README.md#messaging) for the moderation workflow and Spam display policy                                             |
...
| XEP                                                   | Name                                          | Status        | Notes                                                                                                                                                |
|-------------------------------------------------------|-----------------------------------------------|---------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| [XEP-0373](https://xmpp.org/extensions/xep-0373.html) | OpenPGP for XMPP (OX)                        | ✅ Implemented | Multi-key public-key publication and retrieval via PEP node (`urn:xmpp:openpgp:0:public-keys`), secret key backup (XEP-0223 private node), and unverified-keyset status when a verified contact announces an additional active key |
| [XEP-0374](https://xmpp.org/extensions/xep-0374.html) | OpenPGP for XMPP Instant Messaging (OX-IM)   | ✅ Implemented | `<signcrypt>` envelope for 1:1 and MUC messages; processes incoming payload extensions from compatible clients (corrections, replies and fallbacks, receipts, markers, chat states); send-policy negotiation; MAM history decryption; capability discovery via caps (`urn:xmpp:openpgp:im:0`, advertised only while OpenPGP is enabled) |
