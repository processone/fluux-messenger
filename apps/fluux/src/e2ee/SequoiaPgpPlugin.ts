...
export class SequoiaPgpPlugin implements E2EEPlugin {
   ...
   * the server's published key is wrong — either because it was tampered
   * with, or because the local key is the one the user wants to keep (e.g.
   * the other device's key is now stale).
   *
   * On success, clears the `ownKeyConflictStore` so encryption resumes.
   * Throws on publish failure; the conflict stays live so the user can
   * retry.
   */
  async resolveOwnKeyConflict_overwriteServer(): Promise<void> {
    if (!this.ownBundle) {
      throw new E2EEPluginError(
        'permanent',
        'no-identity',
        'SequoiaPgpPlugin: cannot overwrite server before ensureIdentity has completed',
      )
    }
  ...
  /**
   * Resolve an own-key conflict by importing the secret-key backup from
   * the server's private PEP node (`urn:xmpp:openpgp:0:secret-key`),
   * replacing the locally-held key with the server's version. Use this
   * when another device published a newer key that this device should
   * adopt — for example after a subkey rotation or a full re-keying on a
   * different device.
   *
   * Delegates to `restoreSecretKey`, which handles decryption, local
   * persistence, and re-advertising the restored public key to PEP.
   * On success, clears the `ownKeyConflictStore` so encryption resumes.
   * Throws when no backup exists, when the passphrase is wrong, or on
   * a network failure; the conflict stays live so the user can retry.
   */
  ...
  async ensureIdentity(): Promise<IdentityInfo> {
    ...
    if (getOwnKeyConflict()) {
      // A mismatch was detected between the locally-held key and what
      // the server currently advertises. Publishing blindly would
      // overwrite a potentially legitimate key published by another of
      // the user's devices without them having had a chance to decide.
      // Encryption is blocked via `encrypt()` until the user resolves
      // the conflict through one of the two resolution methods.
      ctx.logger.warn(
        `SequoiaPgpPlugin: own key conflict detected (${getOwnKeyConflict()!.kind}); ` +
  ...
  /**
   * Two-level consistency check between the locally-held key and what
   * the server currently publishes under our JID.
   *
   * Level 1 — metadata node: compare the advertised primary fingerprint
   * against the local key. A mismatch means the server has a completely
   * different key (tampering, or a new key generated by another device).
   *
   * Level 2 — data node: same primary fingerprint in the metadata but
   * different armored content in the data node. Typical cause: another
   * device ran `rotateEncryptionKey()` and published a new encryption
   * subkey that this device hasn't imported yet.
   *
   * On any detected mismatch, calls `recordOwnKeyConflict()` and
   * returns. On consistency (or first publish — empty metadata node),
   * calls `clearOwnKeyConflict()`. Never throws; any PEP query failure
   * is treated as "unknown, proceed normally" to avoid blocking init on
   * transient server issues.
   */
  private async checkOwnPublishedKeyConsistency(bundle: KeyBundle): Promise<void> {
    const ctx = this.requireCtx()

    // Level 1: metadata node — primary fingerprint check.
    ...
    // Extract the publish date from the first <pubkey-metadata> element
    // for surfacing in the conflict UI.
    let publishedDate = ''
    ...
    // Level 2: data node — full armored content check.
    // Catches subkey rotations done on another device (same primary FP,
    // different encryption subkey in the published data node).
  ...
  async encrypt(handle: ConversationHandle, plaintext: Uint8Array): Promise<EncryptedPayload> {
    const ctx = this.requireCtx()
    // Hard-block when our own published key is inconsistent with the
    // local key. Encrypting while the conflict is live risks sending
    // messages that can't be decrypted by our contacts (if they have
    // our old server key) or that silently rely on an untrusted key.
    // The user must resolve via `resolveOwnKeyConflict_overwriteServer`
    // or `resolveOwnKeyConflict_importFromServer` before encryption
    // can resume.
    if (getOwnKeyConflict()) {
      throw new E2EEPluginError(
        'permanent',
        'own-key-conflict',
        `SequoiaPgpPlugin: own key conflict (${getOwnKeyConflict()!.kind}) must be resolved before encrypting`,
      )
    }
