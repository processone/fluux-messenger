...
describe('key expiration (XEP-0373 §5 cross-client restore)', () => {
  // Gajim's `OpenPGP.import_key` refuses outright any restored secret key
  // whose primary carries an expiration ("Imported key has expiration
  // date"). The expiry lives in the key's own self-signatures rather than
  // in the backup blob, so re-publishing the backup cannot fix it — the
  // key has to be re-signed. openpgp.js happens to default to no expiry,
  // but a key can still arrive with one (imported from GnuPG or
  // OpenKeychain, or generated by an older build), so the web backend
  // pins the same guarantee the Sequoia backend does.

  const PASSPHRASE = 'hunter2-strong-passphrase'

  /** Expiration of the primary key: `Infinity` when it never expires. */
  async function primaryExpiration(armored: string): Promise<Date | typeof Infinity | null> {
    const { readKey } = await import('openpgp')
    return (await readKey({ armoredKey: armored })).getExpirationTime()
  }

  /** Seed storage with a key shaped like one that carries a legacy expiry. */
  async function seedExpiringKey(ctx: PluginContext, jid: string): Promise<string> {
    const { generateKey, encryptKey } = await import('openpgp')
    const { privateKey } = await generateKey({
      type: 'ecc',
      curve: 'curve25519Legacy',
      userIDs: [{ name: `xmpp:${jid}` }],
      keyExpirationTime: 3 * 365 * 24 * 60 * 60,
      format: 'object',
    })
    const encrypted = await encryptKey({ privateKey, passphrase: PASSPHRASE })
    await ctx.storage.put('private-key', new TextEncoder().encode(encrypted.armor()))
    return privateKey.getFingerprint()
  }

  afterEach(() => clearSessionPassphrase())

  it('generates keys that never expire', async () => {
    const plugin = new TestableWebOpenPGPPlugin()
    const { ctx } = makeCtx('alice@example.com')
    setSessionPassphrase(PASSPHRASE)
    await plugin.init(ctx)
    ...
    expect(await primaryExpiration(bundle.publicArmored)).toBe(Infinity)
  })

  it('clears the expiration on a stored key when unlocking it', async () => {
    const { ctx } = makeCtx('alice@example.com')
    const fingerprint = await seedExpiringKey(ctx, 'alice@example.com')
    // Guard: the fixture must really carry an expiry, else this proves nothing.
    const stored = await ctx.storage.get('private-key')
    expect(await primaryExpiration(new TextDecoder().decode(stored!))).not.toBe(Infinity)
    ...
    expect(await primaryExpiration(bundle.publicArmored)).toBe(Infinity)
    expect(bundle.fingerprint).toBe(fingerprint)
  })

  it('clears the expiration on subkeys too, not just the primary', async () => {
    // Clearing only the primary would satisfy Gajim's gate but leave
    // encryption breaking three years in, when the [E] subkey lapses.
    const { readKey } = await import('openpgp')
    const { ctx } = makeCtx('alice@example.com')
    await seedExpiringKey(ctx, 'alice@example.com')
    ...
    const subkeys = (await readKey({ armoredKey: bundle.publicArmored })).getSubkeys()
    expect(subkeys.length).toBeGreaterThan(0)
    for (const subkey of subkeys) {
      expect(await subkey.getExpirationTime()).toBe(Infinity)
    }
  })

  it('rewrites storage so the cleared expiration survives a reload', async () => {
    const backend = new InMemoryStorageBackend()
    const { ctx } = makeCtx('alice@example.com', backend)
    await seedExpiringKey(ctx, 'alice@example.com')
    ...
    // A fresh plugin over the same backend must see the healed key on disk,
    // not heal it again from a still-expiring blob.
    clearSessionPassphrase()
    const second = new TestableWebOpenPGPPlugin()
    const { ctx: ctx2 } = makeCtx('alice@example.com', backend)
    const persisted = await ctx2.storage.get('private-key')
    expect(await primaryExpiration(new TextDecoder().decode(persisted!))).toBe(Infinity)

    setSessionPassphrase(PASSPHRASE)
    await second.init(ctx2)
    const bundle = await second.callEnsureKeyMaterial('alice@example.com')
    expect(await primaryExpiration(bundle.publicArmored)).toBe(Infinity)
  })

  it('still signs and decrypts after the expiration is cleared', async () => {
    // Re-signing must not damage the key.
    const { ctx } = makeCtx('alice@example.com')
    await seedExpiringKey(ctx, 'alice@example.com')
    const alice = new TestableWebOpenPGPPlugin()
    setSessionPassphrase(PASSPHRASE)
    await alice.init(ctx)
    const aliceBundle = await alice.callEnsureKeyMaterial('alice@example.com')
    // Anchor the test to the healed key — otherwise it would pass just as
    // well against an untouched one and prove nothing about re-signing.
    expect(await primaryExpiration(aliceBundle.publicArmored)).toBe(Infinity)
    ...
    const out = await alice.callDecryptWithOwnKey(
      'alice@example.com',
      ciphertext,
      aliceBundle.publicArmored,
    )

    expect(out.plaintext).toBe('healed and working')
