...
it('offers individual and sender moderation for an existing cache entry without authority metadata', async () => {
  ...
  expect(screen.getByRole('button', { name: /Remove multiple messages from Alice/ })).toBeInTheDocument()
...
describe('message delete dialog bulk entry', () => {
  it.each(['Spam', 'Off topic'])('carries the chosen reason %s into sender moderation', reason => {
    const props = listProps()
    const onModerateSender = vi.fn()
    render(<RoomMessageList {...props} onModerateSender={onModerateSender} />)
    fireEvent.click(screen.getByRole('button', { name: 'Delete test message' }))
    fireEvent.change(screen.getByRole('textbox'), { target: { value: `  ${reason}  ` } })
    fireEvent.click(screen.getByRole('button', { name: /Remove multiple messages from Alice/ }))
    expect(onModerateSender).toHaveBeenCalledWith(target, reason)
  ...
  it('offers sender review without deleting and passes the exact message identity', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Delete test message' }))
    fireEvent.click(screen.getByRole('button', { name: /Remove multiple messages from Alice/ }))
    expect(onModerateSender).toHaveBeenCalledWith(target, undefined)
    expect(props.moderateMessage).not.toHaveBeenCalled()
    expect(screen.queryByRole('button', { name: /Remove multiple messages from Alice/ })).not.toBeInTheDocument()
  })
  ...
  it('keeps the existing single-message removal', () => {
    ...
    expect(screen.getByRole('checkbox', { name: /Spam.*hide/ })).toHaveAccessibleDescription(en.rooms.moderationSpamHint)
    fireEvent.click(screen.getByRole('button', { name: 'Remove message' }))
  ...
  it('does not group an author using only a recyclable nickname', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Delete test message' }))
    expect(screen.queryByRole('button', { name: /Remove multiple messages from/ })).not.toBeInTheDocument()
