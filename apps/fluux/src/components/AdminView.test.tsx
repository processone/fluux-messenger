...
describe('AdminView mobile section menu', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('opens the section menu from the header menu button', () => {
    render(<AdminView activeCategory="rooms" onBack={vi.fn()} />)

    // The menu (and its Users section button) is not rendered until opened.
  ...
  it('navigates and closes the menu when a main-content section is chosen', () => {
    render(<AdminView activeCategory="rooms" onBack={vi.fn()} />)
    ...
    // Menu closed → its section buttons are gone again.
    expect(screen.queryByText('admin.categories.users')).not.toBeInTheDocument()
  })
})
