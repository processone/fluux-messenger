...
export const changelog: ChangelogEntry[] = [
  {
    ...
    sections: [
      ...
      },
      {
        type: 'fixed',
        items: [
          'Your read position now reaches your other devices in the cases where it used to stall — after you reply in a one-to-one chat, in conversations with nothing currently loaded, after a position that failed to publish once, or when older history has been cleared. Other clients stop showing a stale new-messages divider and an inflated unread badge',
          ...
          'The view stays at the newest message when a link preview, reaction or attachment grows the last row after it has rendered, or when the composer shrinks again after sending',
          ...
          'Messages received late, for example after reconnecting or from another device, now stay in the right chronological order',
          ...
          'Contact suggestions in their dialog now use a single, smooth scroll area',
          'Deleting a message also removes it from search results; conversation and search previews now say “Message deleted” rather than showing its old text',
          'A profile picture disappears when its owner removes it, rather than lingering as a stale image',
          ...
          'Copying several messages now includes polls and messages that only contain files; quoted-message and reply cards stay visible when their message is part of a bulk selection, sidebar and search previews keep code spans literal instead of stripping the formatting inside them, and the frosted background of modal panels paints correctly',
          'Connecting to a server that has moved to a new address is more reliable, and accounts with non-ASCII passwords can sign in correctly',
        ],
      },
    ],
  },
