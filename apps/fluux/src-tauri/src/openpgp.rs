...
mod tests {
    ...
    fn generate_sequoia_golden_vectors() {
        ...
        let ciphertext = state
            .encrypt("alice@example.com", &bob.public_armored, plaintext)
            .unwrap();
  ...
  "backupPassphrase": "{}"
    ...
    /// Requires `packages/openpgp-plugin/src/fixtures/web_*.asc` to exist —
    /// run the TS generator first:
    ///   cd packages/openpgp-plugin && npx vitest run src/generateWebVectors.manual.test.ts
    #[test]
    #[ignore]
    fn consume_web_golden_vectors() {
        ...
        if !meta_path.exists() {
            panic!(
                "Web golden vectors not found at {}. \
                 Run `cd packages/openpgp-plugin && npx vitest run src/generateWebVectors.manual.test.ts` first.",
                meta_path.display()
            );
        }
        ...
        let output = bob_state
            .decrypt("bob@example.com", &ciphertext, Some(&alice_pub))
