...
| 2.4 | **Per-conversation text drafts already work**: typing in A, switching to B, switching back restores the text (implemented via `useConversationDraft` + the persisted `chatStore.drafts` map). The remaining gap: a **staged attachment** (file dropped/picked but not yet sent) is cleared on conversation switch, because `ChatView.tsx` revokes its preview URL and calls `setPendingAttachment(null)` when `activeConversation.id` changes.                                                                                                             | Hold staged attachments per JID too (keep the `File` + preview URL keyed by conversation, restore on switch). The deferred upload-on-Send is intentional for privacy; only the cross-switch persistence is missing. The text-draft half is already done.                                                                                                                                     | L   | M   |
...
| #   | Issue                                                                                                                                                                                                | Recommendation                                                                                                                                                  | Sev | Eff |
|-----|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|-----|-----|
| 7.1 | ✅ **Resolved.** The Web Push row is now conditionally rendered: hidden where unsupported, shown with proper status/actions where available, and on Tauri it links to system notification settings.                                                    | Shipped: `NotificationsSettings.tsx:227-280`.  | M   | S   |
...
with the corresponding `events-list` capture in `e2e/ux-screenshots.ts`.
...
The app is i18n-aware (33 languages per `e2e/screenshots.ts`). The screenshots
...
[x] [H] Surface subscription requests / room invitations without a tab visit: §9 (Events tab dissolved in #789: requests inline in contacts, invitations inline in Rooms, reactions in context; findings §9.1 to §9.4 dropped with the screen)
...
- Screenshots can be regenerated by running `npm run ux:audit`
  (`e2e/ux-screenshots.ts` + `e2e/playwright.ux.config.ts`). Re-run before
