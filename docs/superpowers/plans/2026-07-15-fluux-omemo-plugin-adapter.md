...
- **Runtime deps:** `@fluux/omemo`, `@fluux/sdk`, `@xmpp/xml` (workspace `*` for the two `@fluux/*`). No app import, no `@tauri-apps/*`.
- **The core `@fluux/omemo` stays XML-free / `@noble`-only.** Only this plugin package takes the `@xmpp/xml` dependency.
