...
- **PR B derives `unreadCount` ONLY. Archive recounts must NEVER write `mentionsCount`.** The live `+1` path keeps maintaining mentions, and explicit read / mark-read stays responsible for clearing them. Rationale: `isMention` is set only on the live stanza path (`Chat.ts:2272`); MAM's `RoomMessage` constructor never sets it and nothing recomputes it, so an archive scan sees `mentions: 0` for every offline-arrived mention. Letting a scan write that value would zero a correctly live-counted mention — a new regression. There is therefore **no `mentions`, no `mentionsComplete`, and no mention scan cap** anywhere in the archive primitive; do not leave unused machinery behind. `unreadCount` still caps at 999.
  *Deferred follow-up (NOT this PR):* persist a trustworthy mention classification at ingest — classification reflecting the nickname/identity applicable when the message was received. Only once that field is trustworthy may an archive scan raise or lower `mentionsCount`.
...
- **Round-7 (plan-owner re-scope, post-Task-4 review):** the Task 4 review established that `isMention` is set ONLY on the live stanza path — MAM never sets it — so an archive scan reports `mentions: 0` for every offline-arrived mention, and the old "a complete scan may lower mentionsCount" rule would have zeroed correctly live-counted mentions. **PR B is now scoped to `unreadCount` only:** `mentions` / `mentionsComplete` / the mention scan cap are removed from the archive primitive and from the transient overlay (no unused machinery), archive recounts never write `mentionsCount`, and Tasks 7/8 must prove that `exact` / `deferred` / `unavailable` recounts all leave it unchanged. Trustworthy mention derivation is deferred to a follow-up that persists a mention classification at ingest. Separately, `mergeRoomRows` now ORs `isMention` independently of content ownership (monotonic evidence: an absent flag must never erase an established `true`), tested in both row orders.
