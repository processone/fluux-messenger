...
- **The canonical count = archived count + a position-aware transient overlay.** Eligible `noLocalStore` messages are never written to IndexedDB (`message-internal.ts:26`; set by `MUC.ts:256`, `roomStore.ts:1526`) yet must count as unread — existing tests assert it (`chatStore.test.ts:1403`, `roomStore.test.ts:1408`). The overlay is scoped by `{accountScope, kind, entityId}`, identified through **B0's tiered room identity** (`roomMessageIdentity.ts`: `stanzaId → originId → from+id`, indexing every alias — never a bare id and never a `from+id`-only key), stores each entry's `OrderPosition` + `isMention`, and is **never cleared on deactivation** — only when the pointer passes an entry, the message is removed, or the account is torn down.
...
- **Mentions have their own completeness contract.** `unreadCount` caps at 999; the same cursor keeps walking for mentions to a separate hard bound (5000 rows) and reports `mentionsComplete`. When `mentionsComplete === false` the result may **never lower** the trusted value — commit `max(existing, scanned)`; only a complete scan may reduce `mentionsCount`. The unread result still commits independently.
...
### Task 4: Archive count primitive with mention-completeness
...
export interface UnreadCountArgs {
  ...
  mentionScanCap?: number // default 5000 rows scanned
}
export interface ArchiveCount { unread: number; mentions: number; mentionsComplete: boolean }
...
**Behavior:** cursor from `floor` forward (chat `conv_timestamp`; room `room_ts_from_id`), counting rows that are `!isOutgoing && isRenderableStoredMessage(row)` **strictly after** the pointer position via `compareOrder` (missing `archiveOrderKey` ⇒ strict-after-timestamp — over-counts, safe). `unread` stops incrementing at `unreadCap` but **the walk continues** tallying `mentions` until the range is exhausted or `mentionScanCap` rows have been scanned. `mentionsComplete = true` iff the range was exhausted (not stopped by the scan cap). Chat has no `isMention` field → `mentions: 0, mentionsComplete: true`. `null` on IndexedDB error.

- [ ] **Step 1: Failing tests** — (a) counts renderable incoming after the pointer; (b) excludes outgoing; (c) excludes non-renderable; (d) **same-ms**: pointer at `m1@t`, unread `m2@t` later by `(from,id)` → 1; (e) unread saturates at `unreadCap` while the walk continues; (f) **a mention beyond the unread cap is still counted, `mentionsComplete: true`**; (g) hitting `mentionScanCap` → `mentionsComplete: false`; (h) missing `archiveOrderKey` → strict-after-timestamp. Example (f):

```ts
it('counts a mention beyond the unread cap and reports completeness', async () => {
  // 1200 unread room messages, the 1100th is a mention; unreadCap 999, scan cap 5000
  const res = await messageCache.countRoomUnreadInArchive('r@c', { floor: new Date(0), unreadCap: 999 })
...
- [ ] **Step 4: Run → PASS.**
- [ ] **Step 5: Deliberate-break ×3** — (a) `>=` instead of strict-after → test (d) becomes 2 and FAILS; (b) drop the `isOutgoing` filter → (b) FAILS; (c) `break` out of the walk at `unreadCap` → test (f) reports `mentions: 0` and FAILS. Revert each.
- [ ] **Step 6: Commit** — `feat(cache): archive count primitive with independent mention scan + completeness`
...
export interface TransientEntry { position: OrderPosition; isMention: boolean }
...
export function transientCounts(key: ScopeKey, boundary: OrderPosition | undefined): { unread: number; mentions: number }
...
> 3. **Entries carry their `OrderPosition` and `isMention`**, so they can be compared against the read boundary and so transient **mentions** are counted too — not just unread.
...
- `noteTransient()` first resolves **every** supplied alias through `canonicalByAlias`. If they resolve to **more than one** existing canonical entry, it **coalesces them into one** (union the alias sets, keep the earliest `position`, OR the `isMention` flags, re-point every alias at the survivor) rather than picking the first — a higher identity tier arriving later can bridge two entries that were previously distinct, exactly as B0's `upsertStoredRoomRow` merges every identity match. Its result distinguishes three cases:
  ...
  - **coalesced two or more entries, flipped `isMention`, or moved the retained `position`** → `{ added: false, requiresRecount: true }` — **schedule the entity recount**, because the overlay's contribution changed even though no message was added (coalescing `2 → 1` is the canonical case: the fast path correctly declines to increment, but the stored count is now stale by one).
...
Tasks 7/8 commit `unread = min(999, archive.unread + transient.unread)` and fold `transient.mentions` into the mention rule.
...
const note = (m: typeof msg & { stanzaId?: string }, at: number, isMention = false) =>
  noteTransient(K, { position: { timestamp: at }, isMention },
...
it('coalesces two entries when a later alias bridges them', () => {
  ...
  noteTransient(K, { position: { timestamp: 10 }, isMention: false }, 'origin-key-O', ['origin-key-O'])
  noteTransient(K, { position: { timestamp: 10 }, isMention: false }, 'stanza-key-S', ['stanza-key-S'])
  expect(transientCounts(K, { timestamp: 5 }).unread).toBe(2)
  const r = noteTransient(K, { position: { timestamp: 10 }, isMention: false },
...
it('counts transient mentions separately', () => {
  const k = { accountScope: 'me@x', kind: 'room' as const, entityId: 'r@c' }
  noteTransient(k, { position: { timestamp: 10 }, isMention: true }, 'a')
  expect(transientCounts(k, { timestamp: 5 })).toEqual({ unread: 1, mentions: 1 })
})
```
...
- [ ] **Step 4: Wire** — `noteTransient` from the `+1` path when the message is `noLocalStore` **and** renderable (Task 9), carrying its `OrderPosition` + `isMention`, **incrementing the store only on `added: true` and scheduling a recount when `requiresRecount` is true**; `pruneTransient` on pointer advance; `removeTransient` on retraction/removal, **scheduling a recount when `removed` is true**; `clearTransientScope` on account teardown/scope switch. **Do not** hook deactivation.
...
7. **Latest-wins commit:** re-read the version; if `recountVersion.get(id) !== v`, **discard** (a newer recount owns the result). Otherwise:
   ...
   - **Mentions — an incomplete scan may never LOWER the trusted value.** Let `scanned = res.mentions + transient.mentions`. Commit `res.mentionsComplete ? scanned : Math.max(existingMentions, scanned)`. Only a **complete** scan may reduce `mentionsCount`. (Protecting only the `scanned === 0` case is insufficient: a trusted `5` with an incomplete scan finding `2` would wrongly commit `2`.) Unread still commits independently of mention completeness.
...
Increment `unreadCount` only for a message the shared `isRenderableStoredMessage` predicate accepts (re-export it from `readState.ts` so the live path and the cache walk cannot drift). When the accepted message is `noLocalStore`, call **`noteTransient`** (Task 6) with its `OrderPosition`, `isMention`, and the identity + aliases from `transientIdentity`/`transientAliases`, and **increment `unreadCount` only when it returns `added: true`**, while **scheduling a recount when it returns `requiresRecount: true`** — an alias-merge of an already-known message must not increment a second time, but a merge that coalesced entries or changed mention/position must still refresh the projection.
...
- [ ] **Steps:** delete the `MAM_POINTER_RECOUNT_CACHE_LIMIT`-bounded recount blocks (superseded by Tasks 7/8); keep `recomputeCountsFromPointer` itself (PR C removes it). Grep the constant; delete if unused. Full suite green. Commit `refactor(read-state): drop the slice-limited recount`.
...
### Task 12: Single-source the UI count — four surfaces, live-track divider
...
```
> **`>= 999`, not `> 999`** — the stored count saturates at 999 (Task 4), so it never reaches 1000; `> 999` would render a saturated "999 or more" as an exact "999". Treating 999 as "999+" is consistent across all four surfaces.
...
- **Round-2 review items closed:** (a) **T6 overlay is no longer lossy** — scoped `{accountScope, kind, entityId}`, canonical room identity (`from`+`id`), entries carry `OrderPosition` + `isMention`, **never cleared on deactivate** (only pointer-passed / removed / account teardown), transient mentions counted; four new tests incl. same-id-different-sender and partial pointer advance. (b) **Incomplete mention scans can no longer lower the trusted count** — `max(existing, scanned)` when `!mentionsComplete`, not just the zero case. (c) **One generation owner** — the SDK activation path alone calls `beginViewportGeneration`; views only read it. (d) **Full numeric-surface inventory** — `ConversationList` (raw today), `JumpToLastReadPill` (formats itself), divider, FAB badge each get a `998/999/1000` test; `RoomsList` unread is a dot (no formatter) and its `@mentions` badge is a separate quantity. (e) **Anchor capture point named** — a layout effect keyed on the new divider id reads post-mutation geometry, so the plan uses a continuously-maintained anchor + pre-paint `restoreAnchor`.
- **Ordering:** T1→T2→T3→T4→T5→T6→T7→T8→T9→T10→T11→T12. T6 must precede T9 (which registers into it) and T7/T8 (which sum it).
- **Type consistency:** `ArchiveOrderKey`/`compareOrder`/`OrderPosition` (T1) reused verbatim in T2/T3/T4/T5; `ArchiveCount.mentionsComplete` (T4) consumed by the T7/T8 commit rule; `EvidenceKey` (T11) used by both views.
