...
Stage 2 (per-identity trust surface) gets its own plan, authored against the real Stage-1 code — its `resolvePeerTrust`, verified-set store, and `verificationSync` `:1` migration depend on the exact shapes committed here, so writing it now would invite type-drift.
